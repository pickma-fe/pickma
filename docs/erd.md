...
현행 스키마에는 포함되지 않은 후속 task 예정 변경 후보다. 세부 컬럼명, 타입, 구현 방식은 각 task 계획 시점에 확정한다. 실제 migration 파일은 각 기능 task에서 작성한다.

| 후보 주제                     | 관련 Task | 대상 테이블 | 변경 내용 요약                                                                                                                                               |
| ----------------------------- | --------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 판매자 운영 상태 컬럼         | T10       | `stores`    | 운영 상태 컬럼 추가. `is_open` boolean 또는 `operation_status` enum 중 선택은 T10에서 결정. 관련 RLS/index 포함.                                             |
| 위치 저장 및 거리 계산 전략   | T21       | `stores`    | 위치 데이터 저장 방식 결정. `latitude`/`longitude` 컬럼 추가, 외부 거리 API, PostGIS/RPC 등 방식은 T21에서 결정.                                             |
| 상품 DB 레벨 filter/sort 계산 | T05       | `products`  | `discountRate`, `availableStock`, `displayStatus`를 DB 레벨에서 filter/sort/range 가능하게 하는 방식 검토. view/RPC/generated column 등 방식은 T05에서 결정. |

## 비차단 후보 (T11 이후 결정)

| 후보 주제   | 관련 Task | 비고                                                                                                                                                                                 |
| ----------- | --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 알림 스키마 | T22       | T22의 직접 선행 task는 T11이며, 알림 저장 방식(`notifications` 테이블 또는 Realtime 채널)은 T11 이후 T22 계획 시점에 결정한다. 현행 스키마 및 예정 migration 목록에 포함하지 않는다. |
