...
- 배경:
  IA 정합성 확인 중 소비자 프로필 수정 화면이 미구현 상태임을 확인했다. PRD C-MY-03 요구사항에 해당한다. T54에서 마이페이지 기본 화면을 내 정보로 정리하면서 최종 진입 경로는 `/mypage`로 통합했다.
...
- 완료 기준:
  - `/mypage`에서 닉네임과 연락처를 수정하고 저장할 수 있다.
  - 저장 후 `/mypage` 현재 페이지에 머무른다.
  - 실제 로그인 provider가 있으면 내 정보 화면에 해당 로그인 방식을 표시한다.
  - 회원 탈퇴 시 계정이 비활성화되고 로그아웃된다.
  - 비로그인 사용자는 `/mypage` 접근 시 로그인 guard 흐름을 따른다.

- 구현 결과:
  - `/mypage` 내 정보 화면과 `ProfileEditPageContent` 구현.
