...
- 확인 필요 사항:
  - 주문 취소 버튼은 T31 취소 API 구현 전까지 disabled 또는 미노출로 유지하고, T31 이후 활성화 조건을 정리한다.
  - 결제 대기/만료 주문의 표시 방식

- 구현 결과:
  - `/mypage` 기본 진입 화면을 내 정보 화면으로 정리했다.
  - `/mypage/orders`에서 기존 내 예약 목록, 상세 모달, 픽업 코드 확인 흐름을 유지하도록 분리했다.
  - `/mypage/profile` 중복 진입 경로를 제거했다.
  - 예약 완료/실패 후 “내 예약 보기” 링크를 `/mypage/orders`로 연결했다.
  - IA와 task 문서를 실제 route 구조에 맞게 갱신했다.
