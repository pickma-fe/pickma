...
- 완료 기준:
  ...
  - `public/icons/icon-192x192.png`, `public/icons/icon-512x512.png`: 앱 아이콘 추가
  - `public/sw.js`: 서비스 워커 직접 구현 (네트워크 우선 캐싱, Push 이벤트 수신)
  - `src/components/common/ServiceWorkerRegister.tsx`: 서비스 워커 등록 컴포넌트
  - `src/hooks/usePushNotification.ts`: 푸시 알림 권한 요청 및 PushSubscription 생성 훅
  ...
  - 실기기 PWA 설치 및 standalone 모드 검증: Vercel 배포 후 확인
