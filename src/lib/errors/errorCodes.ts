export const ERROR_CODE = {
  UNAUTHORIZED: 'UNAUTHORIZED',
  FORBIDDEN: 'FORBIDDEN',
  VALIDATION_ERROR: 'VALIDATION_ERROR',
  NOT_FOUND: 'NOT_FOUND',
  PRODUCT_NOT_FOUND: 'PRODUCT_NOT_FOUND',
  ORDER_NOT_FOUND: 'ORDER_NOT_FOUND',
  STORE_NOT_FOUND: 'STORE_NOT_FOUND',
  CATEGORY_NOT_FOUND: 'CATEGORY_NOT_FOUND',
  MENU_ITEM_NOT_FOUND: 'MENU_ITEM_NOT_FOUND',
  MENU_ITEM_INACTIVE: 'MENU_ITEM_INACTIVE',
  STORE_INACTIVE: 'STORE_INACTIVE',
  STORE_ALREADY_EXISTS: 'STORE_ALREADY_EXISTS',
  OUT_OF_STOCK: 'OUT_OF_STOCK',
  PRODUCT_EXPIRED: 'PRODUCT_EXPIRED',
  PRODUCT_NOT_AVAILABLE: 'PRODUCT_NOT_AVAILABLE',
  INVALID_ORDER_STATUS: 'INVALID_ORDER_STATUS',
  ORDER_EXPIRED: 'ORDER_EXPIRED',
  PAYMENT_AMOUNT_MISMATCH: 'PAYMENT_AMOUNT_MISMATCH',
  PAYMENT_CONFIRM_FAILED: 'PAYMENT_CONFIRM_FAILED',
  PAYMENT_CANCEL_FAILED: 'PAYMENT_CANCEL_FAILED',
  PAYMENT_ALREADY_CONFIRMED: 'PAYMENT_ALREADY_CONFIRMED',
  INVALID_WEBHOOK_PAYLOAD: 'INVALID_WEBHOOK_PAYLOAD',
  DUPLICATE_PRODUCT_IN_ORDER: 'DUPLICATE_PRODUCT_IN_ORDER',
  ORDER_NUMBER_EXHAUSTED: 'ORDER_NUMBER_EXHAUSTED',
  PICKUP_NUMBER_EXHAUSTED: 'PICKUP_NUMBER_EXHAUSTED',
  AUTH_IDENTITY_CONFLICT: 'AUTH_IDENTITY_CONFLICT',
  SELLER_APPLICATION_NOT_FOUND: 'SELLER_APPLICATION_NOT_FOUND',
  APPLICATION_ALREADY_SUBMITTED: 'APPLICATION_ALREADY_SUBMITTED',
  SELLER_ALREADY_REGISTERED: 'SELLER_ALREADY_REGISTERED',
  APPLICATION_DOCUMENT_NOT_FOUND: 'APPLICATION_DOCUMENT_NOT_FOUND',
  APPLICATION_CANCEL_NOT_ALLOWED: 'APPLICATION_CANCEL_NOT_ALLOWED',
  FILE_UPLOAD_NOT_ALLOWED: 'FILE_UPLOAD_NOT_ALLOWED',
  FILE_TYPE_NOT_ALLOWED: 'FILE_TYPE_NOT_ALLOWED',
  FILE_TOO_LARGE: 'FILE_TOO_LARGE',
  RATE_LIMIT_EXCEEDED: 'RATE_LIMIT_EXCEEDED',
  AUTH_EMAIL_ALREADY_EXISTS: 'AUTH_EMAIL_ALREADY_EXISTS',
  AUTH_EMAIL_SEND_FAILED: 'AUTH_EMAIL_SEND_FAILED',
  AUTH_EMAIL_OTP_EXPIRED: 'AUTH_EMAIL_OTP_EXPIRED',
  AUTH_EMAIL_OTP_INVALID: 'AUTH_EMAIL_OTP_INVALID',
  AUTH_EMAIL_OTP_ATTEMPT_LIMIT_EXCEEDED:
    'AUTH_EMAIL_OTP_ATTEMPT_LIMIT_EXCEEDED',
  AUTH_EMAIL_VERIFICATION_TOKEN_EXPIRED:
    'AUTH_EMAIL_VERIFICATION_TOKEN_EXPIRED',
  AUTH_EMAIL_VERIFICATION_TOKEN_INVALID:
    'AUTH_EMAIL_VERIFICATION_TOKEN_INVALID',
  AUTH_EMAIL_SIGNUP_IN_PROGRESS: 'AUTH_EMAIL_SIGNUP_IN_PROGRESS',
  AUTH_EMAIL_STORE_UNAVAILABLE: 'AUTH_EMAIL_STORE_UNAVAILABLE',
  NOT_IMPLEMENTED: 'NOT_IMPLEMENTED',
  INTERNAL_SERVER_ERROR: 'INTERNAL_SERVER_ERROR',
} as const;

export type ErrorCode = (typeof ERROR_CODE)[keyof typeof ERROR_CODE];
