...
describe('PATCH /api/seller/orders/[orderId]/ready', () => {
  ...
  it('real 모드에서 markSellerOrderReady를 호출한다', async () => {
    ...
    expect(res.status).toBe(200);
    expect(markSellerOrderReady).toHaveBeenCalledWith(STORE_ID, ORDER_ID);
  });

  it('INVALID_ORDER_STATUS이면 409를 반환한다', async () => {
    vi.mocked(isApiMockEnabled).mockReturnValue(false);
    vi.mocked(requireSellerStore).mockResolvedValue(sellerResult);
    vi.mocked(markSellerOrderReady).mockRejectedValue(
