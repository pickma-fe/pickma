...
function GuestCTA() {
  ...
  return (
    <CTACard
      ...
      title="픽마에서 가게를 운영해보세요"
      description="로그인 후 판매자 신청을 완료하면 픽마의 모든 판매 기능을 이용할 수 있어요."
      footer="가게 등록 승인까지 영업일 기준 1~3일이 소요됩니다."
    >
      <BenefitList />
...
function NotAppliedCTA() {
  return (
    <CTACard
      ...
      iconBgClassName="bg-primary-50"
      title="픽마에서 가게를 운영해보세요"
      description="사업자 정보와 서류를 제출하면 심사 후 판매자로 전환돼요."
      footer="가게 등록 승인까지 영업일 기준 1~3일이 소요됩니다."
    >
      <BenefitList />
      <div className="space-y-3">
...
function PendingCTA() {
  return (
    <CTACard
      ...
      title="심사가 진행 중이에요"
      description="제출하신 서류를 검토 중입니다. 승인까지 영업일 기준 1~3일이 소요돼요."
    >
...
function RejectedCTA({ rejectReason }: { rejectReason?: string }) {
  return (
    <CTACard
      ...
      title="판매자 신청이 반려되었어요"
      description="서류를 보완하여 다시 신청해주세요."
    >
      {rejectReason && (
        <div className="mb-8 rounded-xl bg-red-50 p-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="mb-1 text-sm font-medium text-red-800">반려 사유</p>
...
function ApprovedNoStoreCTA() {
  return (
    <CTACard
      icon={<CheckCircle className="text-primary-500 h-8 w-8" />}
      iconBgClassName="bg-primary-50"
      title="판매자 승인이 완료되었어요!"
      description="이제 가게를 등록하면 상품 판매를 시작할 수 있어요."
    >
      <div className="mb-8 rounded-xl bg-green-50 p-4">
        <div className="flex items-start gap-3">
          <CheckCircle className="mt-0.5 h-5 w-5 shrink-0 text-green-500" />
          <p className="text-sm text-green-800">
            가게 정보(상호명, 주소, 운영 시간 등)를 등록하면 바로 운영을 시작할
            수 있어요.
          </p>
        </div>
      </div>
