...
interface AdminDashboardRejectModalProps {
...
export function AdminDashboardRejectModal({
  application,
  isSubmitting,
  onClose,
  onSubmit,
}: AdminDashboardRejectModalProps) {
      ...
      {application && (
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="rounded-md bg-gray-50 p-4 text-sm">
            ...
            <p className="mt-1 text-gray-500">{application.businessCategory}</p>
          </div>
          <label htmlFor="dashboard-reject-reason" className="block">
            ...
            <textarea
              ...
              aria-describedby={
                error ? 'dashboard-reject-reason-error' : undefined
              }
            />
          </label>
          {error && (
            <p
              id="dashboard-reject-reason-error"
              role="alert"
